...
<!--- PUBLIC METHODS --->
	...
	<cffunction name="uploadPhoto" access="public" output="false" returntype="struct" hint="Upload a photo to Flickr">
	...
	<cffunction name="replacePhoto" access="public" output="true" returntype="struct" hint="Replace a photo with a new image">
		<cfargument name="photo"  type="any" required="yes" hint="Absolute path to an image file to upload">
...
<!--- PRIVATE METHODS --->
	<cffunction name="post" access="private" output="false" returntype="struct">
		...
		arguments.params.api_key = variables.api_key;
		// we want a simple json response with no callback wrapper function 
		arguments.params.format="json";
		arguments.params.nojsoncallback=1;
		// if theres a secret, sign the request
		if(len(variables.secret)) {
			arguments.params.api_sig = createApiSig(arguments.params);
		}
		</cfscript>
		
		<!--- see if its in the cache --->
		<cfif isObject(variables.cache) AND variables.cache.existsInCache(arguments.params)>
			...
			<cfif variables.cache.isObjectCache()>
				...
				<cfset result = duplicate(variables.cache.getFromCache(arguments.params)) /> 
			<!--- otherwise get the json string and desrialize it and return that --->
			<cfelse>
				<cfset result = fixJsonResponse(deserializeJson(variables.cache.getFromCache(arguments.params))) />
			</cfif>
		...
		<cfelse>
			<cfhttp url="#variables.urls.rest#" method="post" result="httpreq">
				<cfloop collection="#arguments.params#" item="i">
					<cfhttpparam name="#lcase(i)#" type="formfield" value="#arguments.params[i]#">
				</cfloop>
			</cfhttp>
			
			<!--- store the response in the cache --->
			<cfset cacheResult(arguments.params, httpreq.filecontent) />
	...
	<cffunction name="upload_post" access="private" output="true" returntype="any" 
		...
		<cfset var result = "" />
		<cfset var i = "" />
		<cfset arguments.params.api_key = variables.api_key />
		<!--- we need to take the photo out of the params collection because it doesnt get hashed --->
		<cfif structkeyexists(arguments.params, "photo")>
			<cfset photo = arguments.params.photo />
			<cfset structdelete(arguments.params, "photo") />
		</cfif>
		<cfset arguments.params.api_sig = createApiSig(arguments.params, true) />
		...
		<cfset arguments.params.photo = photo />
	...
	<cffunction name="cacheResult" access="private" output="false" returntype="void" 
		...
		<cftry>
			<cfif isObject(variables.cache)>
				...
				<cfif variables.cache.isObjectCache()>
					<cfset variables.cache.saveToCache(arguments.params, obj) />
				<cfelse>
					<cfset variables.cache.saveToCache(arguments.params, result) />
				</cfif>
			</cfif>
			<cfcatch type="Any">
	...
	<cffunction name="createApiSig" access="private" output="false" returntype="string">
		...
		<cfloop list="#paramlist#" index="i">
			<cfif isDefined("arguments.params.#i#")>
				<cfset sig = sig & i & arguments.params[i] />
			<cfelse>
				<cfset structDelete(arguments.params, i) />
			</cfif>
		</cfloop>
	...
	<cffunction name="fixJsonResponse" access="private" output="false" returntype="any" 
		...
		<cfscript>
		var i = 0;
		if(isStruct(arguments.data) AND structKeyList(arguments.data) IS "_content") {
			return arguments.data._content;
		}
		for(i in arguments.data) {
			if(isStruct(arguments.data[i])) {
				arguments.data[i] = fixJsonResponse(arguments.data[i]);
			}
			else if(isArray(arguments.data[i])) {
				for(var j=1; j<=arraylen(arguments.data[i]); j++) {
					arguments.data[i][j] = fixJsonResponse(arguments.data[i][j]);
				} 
			}
			else if(i IS "_content" AND isSimpleValue(arguments.data[i])) {
				arguments.data["value"] = arguments.data[i];
				structDelete(arguments.data, "_content");
			}
		}
	...
	<cffunction name="addSuccessResult" access="private" output="false" returntype="struct">
		<cfargument name="resp" type="struct" />
		<cfset arguments.resp["success"] = false />
		<cfif structKeyExists(arguments.resp, "stat") AND ucase(arguments.resp.stat) IS "OK">
			<cfset arguments.resp["success"] = true />
		</cfif>
		<cfreturn arguments.resp />
	</cffunction>
